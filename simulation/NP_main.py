...
for current_seed in range(args.train_seed,(args.train_seed + 10)):
    ...
    path_save = "./outputs/np2_%d_%d_%d_%.3f_%.3f_%.3f_%d.npy"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,seed_train)

    path_checkpoint = "./checkpoint/ckpt_np2_%d_%d_%d_%.3f_%.3f_%.3f_%d.pth"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,seed_train)
    ...
    para_model = NP_model.ParametricPart(dx=dx,device=device)
    nonpara_model = NP_model.NonParametricModel(dx=dx,state_dim=state_dim,hidden=hidden,device=device)
    mode = args.mode
    if mode == 'None':
        mode = None
    if mode is None:
