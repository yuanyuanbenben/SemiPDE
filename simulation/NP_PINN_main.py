...
path_checkpoint = "./checkpoint/ckpt_np2_pinn_%d_%d_%d_%.3f_%.3f_%.3f_%d.pth"%(train_curve_num,test_curve_num,
                                                                sample_size,noise_sigma,D_1,D_2,seed_train)
...
PDE_model = PINN_model.NPEquation(state_dim=dim, hidden=hidden).to(device)
...
class Optimize(object):
    ...
    def train_func(self,epoch):
        ...
        for iteration, states in enumerate(self.train):
            if iteration in [0,1,2,3,4,5,6,7]:
                state = states['states']
