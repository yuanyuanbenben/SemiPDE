...
parser.add_argument('--inference','-infer','-in',default = 1, type = int, help='inference index')
...
for current_seed in range(args.train_seed,(args.train_seed + 25)):
    ...
    tau_2 = 1
    niter = 5
    inference_index = args.inference
    nupdate = 64 * 5
    ...
    path_save = "./outputs_para/gfn_variance_%d_%d_%d_%.3f_%.3f_%.3f_%d_%d_%d.npy"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,num_grid[0],inference_index,seed_train)

    path_checkpoint = "./checkpoint_para/ckpt_gfn_variance_%d_%d_%d_%.3f_%.3f_%.3f_%d_%d_%d.pth"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,num_grid[0],inference_index,seed_train)
    tor_1 = 1e-4
    tor_2 = 1e-4
    iteration= 30
    ...
    para_model0 = GFN_model.ParametricPart(dx=(range_grid[0][1]-range_grid[0][0])/num_grid[0])
    # nonpara_model = RD_model.NonParametricModel(state_dim=dim,hidden=hidden)
    nonpara_model0 = GFN_model.NonParametricModel(state_dim=dim,hidden=hidden,device=device)
    checkpoints = torch.load("./checkpoint_para/ckpt_gfn_%d_%d_%d_%.3f_%.3f_%.3f_%d_%d.pth"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,num_grid[0],seed_train))
    ...
    para_model._params['D_1'] = nn.Parameter(torch.tensor(checkpoints['D_1'].to('cpu').detach().numpy()))
    para_model._params['D_2'] = nn.Parameter(torch.tensor(checkpoints['D_2'].to('cpu').detach().numpy()))
    if inference_index == 1:
        para_model0._params['D_1'] = nn.Parameter(torch.tensor(checkpoints['D_1'].to('cpu').detach().numpy()+1e-5))
        para_model0._params['D_2'] = nn.Parameter(torch.tensor(checkpoints['D_2'].to('cpu').detach().numpy()))
    if inference_index == 2:
        para_model0._params['D_1'] = nn.Parameter(torch.tensor(checkpoints['D_1'].to('cpu').detach().numpy()))
        para_model0._params['D_2'] = nn.Parameter(torch.tensor(checkpoints['D_2'].to('cpu').detach().numpy()+5e-5))
    ...
    nonpara_model = dict()
    for i in range(64):
        nonpara_model[str(i)] = GFN_model.NonParametricModel(state_dim=dim,hidden=hidden,device=device)
        nonpara_model[str(i)].net.load_state_dict(copy.deepcopy(checkpoints['net']))
        full_model_dic[str(i)] = GFN_model.SemiParametericModel(para_model,nonpara_model[str(i)],mode=mode,penalty=penalty,device=device)
    # full_model = RD_model.SemiParametericModel(para_model,nonpara_model,mode=mode,penalty=penalty)
    full_model0 = GFN_model.SemiParametericModel(para_model0,nonpara_model0,mode=mode,penalty=penalty,device=device)
    print('==> Model already !')
    ...
    class OptimalProcess(object):
        ...
        def _forward(self, states, t, backward=True):
            ...
            if inference_index == 2:
                loss = self.traj_loss(pred, pred0)*1e10/25
            if inference_index == 3:
                loss = self.traj_loss(pred, pred0)*1e10/26
                ...
                for iteration, data in enumerate(self.train, 0):
                    ...
                    if total_iteration % self.nupdate == 0:
                        with torch.no_grad(): 
                            ...
                            loss_test /= j + 1
                            loss_test = loss_test * (sample_size + 1) / sample_size * 2
