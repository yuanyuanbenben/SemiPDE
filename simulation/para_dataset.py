        ...
        if self.data.get(str(index)) is None:
            ...
            for j in range(self.sample_size+1):
                if j == 0:
                    ...
                    state = pde.FieldCollection.from_scalar_expressions(self.grid,f"0.5*sin({c1}*(1-abs(x))*(1-abs(x))+{c2})+0.5")
...
def create_data(path_train = "./data/train_rd", path_test = "./data/test_rd", path_utest = "./data/utest_rd", 
                ...
                seed_test=None, seed_utest=None, initial=None, noise_sigma=0.1, dim=1):
