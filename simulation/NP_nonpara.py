...
for current_seed in range(args.train_seed,(args.train_seed + 50)):
    ...
    path_train = "./data/np2_train_%d_%d_%d_%.3f_%.3f_%.3f_%d"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,seed_train)
    path_test = "./data/np2_test_%d_%d_%d_%.3f_%.3f_%.3f_%d"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,seed_train)
    path_utest = "./data/np2_utest_%d_%d_%d_%.3f_%.3f_%d"%(train_curve_num,test_curve_num,
                                                                    sample_size,D_1,D_2,seed_train)
    path_save = "./outputs/np2_nonpara_%d_%d_%d_%.3f_%.3f_%.3f_%d.npy"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,seed_train)

    path_checkpoint = "./checkpoint/ckpt_np2_nonpara_%d_%d_%d_%.3f_%.3f_%.3f_%d.pth"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D_1,D_2,seed_train)
    ...
    test_data = torch.from_numpy(shelve.open(path_utest)[str(0)]).float().permute(0,2,3,1).contiguous().view(26*32*32,1)
    noise = torch.tensor(np.random.normal(scale=noise_sigma,size=(26*32*32,1)),dtype=torch.float32)
    print('==> Data already !')
    full_model = NP_model.NonparaModel(state_dim=3,hidden=hidden)

    # optimal
    print('==> Optimal start...')
    optimizer = torch.optim.Adam(full_model.parameters(), lr=tau_1, betas=(0.9, 0.999))
    t_index = np.linspace(0,2.5,26,endpoint=True)
    x_index = np.linspace(-1,1,32,endpoint=False)
