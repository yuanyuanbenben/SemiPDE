...
nepoch = 500
nlog= 4
path_train = "./data/rd_train_%d_%d_%d_%.3f_%.3f_%d"%(train_curve_num,test_curve_num,
                                                                sample_size,noise_sigma,D,seed_train)
path_test = "./data/rd_test_%d_%d_%d_%.3f_%.3f_%d"%(train_curve_num,test_curve_num,
                                                                sample_size,noise_sigma,D,seed_train)
path_utest = "./data/rd_utest_%d_%d_%d_%.3f_%d"%(train_curve_num,test_curve_num,
                                                                sample_size,D,seed_train)
path_checkpoint = "./checkpoint/ckpt_rd_pinn_%d_%d_%d_%.3f_%.3f_%d.pth"%(train_curve_num,test_curve_num,
                                                                sample_size,noise_sigma,D,seed_train)
...
if args.model_mode == 'no_func':
    ...
    path_utest = "./data_paramodel/rd_utest_%d_%d_%d_%.3f_%d"%(train_curve_num,test_curve_num,
                                                                    sample_size,D,seed_train)
    path_checkpoint = "./checkpoint_paramodel/ckpt_rd_pinn2_%d_%d_%d_%.3f_%.3f_%d.pth"%(train_curve_num,test_curve_num,
                                                                    sample_size,noise_sigma,D,seed_train)
...
x_range = np.linspace(-1+1/num_grid,1+1/num_grid,num_grid,endpoint=False) 
position = np.array([[t,x] for t in t_range for x in x_range])
...
class Optimize(object):
    ...
    # Training
    def train_func(self,epoch,para=False):
        print('\nEpoch: %d' % epoch)
        # for i in range(64):
        ...
        for iteration, states in enumerate(self.train):
            ...
            self.PDE_model(self.position,self.solution_net[str(iteration)],para)
        ...
        print(f'D:{self.PDE_model.params["D"]*1e3}e-3')
