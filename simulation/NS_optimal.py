...
class OptimalProcess(object):
    ...
    def run(self):  
            ...
            for iteration, data in enumerate(self.train, 0):
                ...
                if total_iteration % self.nupdate == 0:
                    with torch.no_grad(): 
                        ...
                        parameter_mat1.append(torch.Tensor.detach(torch.Tensor.cpu(self.net.para_model.params["D_1"][0])).numpy().copy())
                        parameter_mat2.append(torch.Tensor.detach(torch.Tensor.cpu(self.net.para_model.params["D_1"][1])).numpy().copy())
                        parameter_mat3.append(torch.Tensor.detach(torch.Tensor.cpu(self.net.para_model.params["D_1"][2])).numpy().copy())
                        parameter_mat4.append(torch.Tensor.detach(torch.Tensor.cpu(self.net.para_model.params["D_2"][0])).numpy().copy())
                        parameter_mat5.append(torch.Tensor.detach(torch.Tensor.cpu(self.net.para_model.params["D_2"][1])).numpy().copy())
                        parameter_mat6.append(torch.Tensor.detach(torch.Tensor.cpu(self.net.para_model.params["D_2"][2])).numpy().copy())
        ...
        return {"train_loss_mat":train_loss_mat,"test_loss_mat":test_loss_mat,"parameter_mat1":parameter_mat1,"parameter_mat2":parameter_mat2
        ,"parameter_mat3":parameter_mat3,"parameter_mat4":parameter_mat4,"parameter_mat5":parameter_mat5,"parameter_mat6":parameter_mat6}